      ...
      models.User.findOne({ 'local.username': socket.request.user, 'local.channels': channelToJoin }).exec()
        .then((user) => {
          if (user) {
            ...
            throw new Error(`error joining channel: ${socket.request.user} is already joined to ${channelToJoin}`);
          } else {
...
module.exports = function setupSocketIo(config, io) {
  io.sockets.on('connection', (socket) => {
    ...
    const connectedClientsForUser = passportSocketIo
      .filterSocketsByUser(io, user => user === socket.request.user);
    ...
    socket.on(events.leave, handleLeave(socket, config, connectedClientsForUser));
    socket.on(events.msg, handleMsg(socket));
    socket.on(events.privateMsg, handlePrivateMsg(socket, io, connectedClientsForUser));
    socket.on(events.disconnect, handleDisconnect(socket, connectedClientsForUser));
  });
};
