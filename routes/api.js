...
router.get('/users', (req, res) => {
  ...
  return models.User.find({}, { 'local.username': 1, 'local.online': 1, _id: 0 }, (err, users) => {
    if (err) {
...
router.get('/user/channels', (req, res) => {
  ...
  return models.User.findOne({ 'local.username': req.user }, { 'local.channels': 1, _id: 0 }, (err, channels) => {
    if (err) {
      res.status(500).json({ error: true });
    } else {
      res.json(channels);
...
router.get('/channel/:name/messages', (req, res) => {
  ...
  return models.User.findOne({ 'local.username': req.user, 'local.channels': req.params.name }).exec()
        .then((user) => {
          if (user) {
            return models.Message.find({ channel: req.params.name }).exec();
          }
          // test depends on this, sadly.
